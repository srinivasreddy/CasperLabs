...
def test_refund_after_session_code_error(payment_node_network):
    ...
    assert (
        later_balance + initial_account1_balance + deploy.cost * CONV_RATE
        == genesis_init_balance
    )
...
def test_not_enough_funds_to_run_payment_code(payment_node_network):
    ...
    deploy = node0.client.show_deploys(latest_block_hash)[0]
    print(f"the error message:  {deploy.error_message}")
    genesis_balance_after_transfer = node0.client.get_balance(
        account_address=GENESIS_ACCOUNT.public_key_hex, block_hash=latest_block_hash
    )
    # assert genesis_balance -  450 * CONV_RATE ? == genesis_balance_after_transfer
    assert genesis_balance - MAX_PAYMENT_COST == genesis_balance_after_transfer
